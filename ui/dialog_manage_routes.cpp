...
DialogManageRoutes::DialogManageRoutes(QWidget *parent) : QDialog(parent), ui(new Ui::DialogManageRoutes) {
    ...
    // --- App selector button (bypassed apps) ---
    auto *appSelectorBtn = new QPushButton(tr("🖥️  Выбрать приложения для проксирования..."), this);
    appSelectorBtn->setStyleSheet(
        "QPushButton { background: #313244; border: 1px solid #89b4fa; border-radius: 6px;"
        "padding: 8px 16px; color: #89b4fa; font-weight: bold; margin-top: 6px; }"
        "QPushButton:hover { background: #45475a; }");
    // Find the main layout and add the button
    if (auto *vlay = qobject_cast<QVBoxLayout *>(layout())) {
        // insert before the last item (dialog buttons)
        vlay->insertWidget(vlay->count() - 1, appSelectorBtn);
    } else {
        layout()->addWidget(appSelectorBtn);
    }
    connect(appSelectorBtn, &QPushButton::clicked, this, [=] {
        // 1. Sync FROM JSON to UI (optional but good for consistency)
        // We look for a rule with "process_name" in the custom JSON.
        QStringList appsInJson;
        auto customObj = QString2QJsonObject(CACHE.custom_route);
        auto rules = customObj["rules"].toArray();
        for (const auto &ruleVal : rules) {
            auto rule = ruleVal.toObject();
            if (rule.contains("process_name")) {
                appsInJson << QJsonArray2QListString(rule["process_name"].toArray());
            }
        }
        
        QStringList current = (SplitLines(NekoGui::dataStore->vpn_rule_process) << appsInJson);
        current.removeDuplicates();

        auto *dlg = new DialogAppSelector(current, this);
        if (dlg->exec() == QDialog::Accepted) {
            auto selected = dlg->getSelectedApps();

            // 1. Save to vpn_rule_process (Internal TUN list)
            NekoGui::dataStore->vpn_rule_process = selected.join("\n");
            NekoGui::dataStore->Save();

            // 2. Sync TO JSON (The "Sync" the user wants)
            // We'll replace/add a rule in the custom JSON for these apps.
            auto match_out = NekoGui::dataStore->vpn_rule_white ? "proxy" : "bypass";
            
            // Remove existing process_name rules to avoid duplicates
            QJsonArray newRules;
            for (const auto &ruleVal : rules) {
                auto rule = ruleVal.toObject();
                if (!rule.contains("process_name")) {
                    newRules.append(ruleVal);
                }
            }
            
            // Add the new consolidated rule at the TOP for highest priority
            if (!selected.isEmpty()) {
                QJsonObject appRule;
                appRule["process_name"] = QList2QJsonArray(selected);
                appRule["outbound"] = match_out;
                newRules.insert(0, appRule);
            }
            
            customObj["rules"] = newRules;
            CACHE.custom_route = QJsonObject2QString(customObj, false);
        }
        dlg->deleteLater();
    });

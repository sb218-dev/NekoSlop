...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    core_path += "nekoslop_core";

    QStringList args;
    args.push_back("nekoslop");
    args.push_back("-port");
    ...
    // Auto-enable TUN mode on startup (always)
        ...
        if (enable) {
            if (NekoGui::dataStore->vpn_internal_tun) {
                ...
                if (requestPermission) {
                    ...
                    auto ret = Linux_Pkexec_SetCapString(NekoGui::FindNekoSlopCoreRealPath(), "cap_net_admin=ep");
                    if (ret == 0) {
                        this->exit_reason = 3;
                        on_menu_exit_triggered();
                    } else {
...
#endif
#ifdef Q_OS_WIN
                    auto n = QMessageBox::warning(GetMessageBoxParent(), software_name, tr("Please run NekoSlop as admin"), QMessageBox::Yes | QMessageBox::No);
                    if (n == QMessageBox::Yes) {
                        this->exit_reason = 3;
                        on_menu_exit_triggered();
                    }
...
inline QJsonArray last_arr; // format is nekoslop_connections_json
...
bool MainWindow::StartVPNProcess() {
    ...
    runOnNewThread([=] {
        ...
        WinCommander::runProcessElevated(QApplication::applicationDirPath() + "/nekoslop_core.exe",
...
bool MainWindow::StopVPNProcess(bool unconditional) {
    if (unconditional || vpn_pid != 0) {
...
#ifdef Q_OS_WIN
        auto ret = WinCommander::runProcessElevated("taskkill", {"/IM", "nekoslop_core.exe",
                                                                 "/FI",
        ...
        p.start("osascript", {"-e", QStringLiteral("do shell script \"%1\" with administrator privileges")
                                        .arg("pkill -2 -U 0 nekoslop_core")});
#else
        if (unconditional) {
            p.start("pkexec", {"killall", "-2", "nekoslop_core"});
        } else {

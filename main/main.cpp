...
#define LOCAL_SERVER_PREFIX "nekosloplocalserver-"
...
int main(int argc, char* argv[]) {
...
#endif
    // Force AppData if in Program Files (standard installation)
    if (QApplication::applicationDirPath().contains("Program Files", Qt::CaseInsensitive)) {
        NekoGui::dataStore->flag_use_appdata = true;
    }
#ifdef NKR_CPP_DEBUG
    ...
    if (NekoGui::dataStore->flag_use_appdata) {
        QApplication::setApplicationName("nekoslop");
    ...
    // RunGuard
    RunGuard guard("nekoslop" + wd.absolutePath());
...
// icons
    ...
    switch (NekoGui::coreType) {
        case NekoGui::CoreType::SING_BOX:
            NekoGui::dataStore->fn = "groups/nekoslop.json";
            break;
        default:
